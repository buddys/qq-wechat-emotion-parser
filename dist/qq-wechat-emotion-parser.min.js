...
if(str.length===0)return[-1];var T=this;var child;var k;var result={};result.arr=[];k=str[pos];child=T.children[k];if(child!==undefined&&pos<str.length){return child.searchOne(str,pos+1);}
...
return result;},search:function(str){if(this.empty==1)return[];var len=str.length;var searchResult=[];var tmp;for(var i=0;i<len-1;i++){tmp=this.searchOne(str,i);if(typeof tmp.arr!=='undefined'&&tmp.arr.length>0){searchResult.push(tmp.arr);i+=tmp.words;}}
...
function qqWechatEmotionParser(str){var indices=trie.search(str);var increment=0;indices.map(function(idx){var pos=idx[0],emotion=emotion_list[idx[1]],img='<img src="'+emotion_map[emotion]+'" alt="'+emotion+'">';str=splice(str,pos+increment,emotion.length,img);increment=increment+img.length-emotion.length;});return str;}
